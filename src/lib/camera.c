    ...
    for (i = 0, camera = &cameraInfo[0], ball = &ballInfo[0]; i < 4; i++, camera++, ball++, r22++)
    {
         ...
         || (camera->flags & (1 << 7)))
        {
            ...
            case 5:
                ...
                {
                    float f1 = 2.0 * ball->currRadius + mathutil_sqrt(mathutil_sum_of_sq_2(sp8.x, sp8.y));
...
void default_camera_env(void)
{
    ...
    mathutil_mtx_copy(mathutilData->mtxB, userWork->matrices[2]);
    mathutil_mtx_copy(cameraInfo[0].unk174, userWork->matrices[3]);
...
void set_current_camera(int cameraId)
{
    struct Camera *camera = &cameraInfo[cameraId];
    Mtx44 projMtx;
    ...
    mathutil_mtx_copy(mathutilData->mtxB, userWork->matrices[2]);
    mathutil_mtx_copy(camera->unk174, userWork->matrices[3]);
    mathutil_mtx_copy(camera->unk1A4, userWork->matrices[0]);
    mathutil_mtx_copy(camera->unk1D4, userWork->matrices[4]);
    mathutil_mtx_copy(userWork->matrices[0], userWork->matrices[1]);
    userWork->matrices[1][0][3] = 0.0f;
    userWork->matrices[1][1][3] = -camera->eye.y;
    userWork->matrices[1][2][3] = 0.0f;
...
void change_current_camera(int cameraId)
{
    if (u_cameraId1 != cameraId || u_cameraId2 != cameraId)
        set_current_camera(cameraId);
}

// almost identical to set_current_camera
void set_current_camera_matrix(int cameraId)
{
    struct Camera *camera = &cameraInfo[cameraId];
    Mtx44 projMtx;
...
{
    if (u_cameraId1 != cameraId)
        set_current_camera_matrix(cameraId);
}
...
void camera_func_13(struct Camera *camera, struct Ball *ball)
{
    ...
    if (!(debugFlags & 0xA) || camera->state != 13)
    {
...
void camera_func_level_main(struct Camera *camera, struct Ball *ball)
{
    ...
    sp28.x = 0.0f;
    sp28.y = 0.0f;
    sp28.z = mathutil_sqrt(mathutil_sum_of_sq_2(3.0f, 1.0f));
...
void camera_func_test(struct Camera *camera, struct Ball *ball)
{
    ...
    f1 = mathutil_sum_of_sq_3(sp10.x, sp10.y, sp10.z);
    if (f1 < 0.09)
    {
...
void camera_func_5(struct Camera *camera, struct Ball *ball)
{
    ...
    sp30.x = camera->eye.x - sp10.pos.x;
    sp30.y = camera->eye.y - sp10.pos.y;
    sp30.z = camera->eye.z - sp10.pos.z;
    ...
    camera->eyeVel.x += f3 * (sp10.pos.x - ball->pos.x);
    camera->eyeVel.y += f3 * (sp10.pos.y - ball->pos.y);
    camera->eyeVel.z += f3 * (sp10.pos.z - ball->pos.z);
...
void camera_func_7(struct Camera *camera, struct Ball *ball)
{
    ...
    sp50.x = camera->eye.x - sp30.pos.x;
    sp50.y = camera->eye.y - sp30.pos.y;
    sp50.z = camera->eye.z - sp30.pos.z;
    ...
    camera->eyeVel.z -= 0.6 * (sp30.pos.z - sp10.pos.z);
...
void camera_func_goal_main(struct Camera *camera, struct Ball *ball)
{
    ...
    f1 = mathutil_sqrt(mathutil_sum_of_sq_2(sp10.x, sp10.z));
    if (f1 > FLT_EPSILON)
    {
...
void camera_func_16(struct Camera *camera, struct Ball *ball)
{
    ...
    if (mathutil_vec_distance(&goal.pos, &ball->pos) < 16.0)
    {
        ...
        if (mathutil_vec_distance(&goal.pos, &sp34.pos) > 2.5 && (rand() & 3) && sp34.pos.y > goal.pos.y)
        {
            double zero;
            ...
            sp60.x = sp34.pos.x - sp28.x;
            sp60.y = sp34.pos.y - sp28.y;
            ...
            camera->eye.z = sp34.pos.z + sp60.z;
        }
        else
        {
            if (f31 > 0.25 && (rand() & 3))
...
void camera_func_26(struct Camera *camera, struct Ball *ball)
{
    ...
    if ((debugFlags & 0xA) && camera->state == 26)
        return;
...
void camera_func_42(struct Camera *camera, struct Ball *ball)
{
    ...
    if ((debugFlags & 0xA) && camera->state == 42)
        return;
...
void func_8001FF2C(struct Camera *camera, struct Ball *ball, Vec *eye, Vec *lookAt, s16 *xrot, s16 *yrot, s16 *zrot, int h)
{
    ...
    for (i = 0, b = &ballInfo[0]; i < 3; i++, b++)  // b not used here. needed to match
    {
        ...
        var1 = mathutil_sum_of_sq_2(sp28.x, sp28.z);
        if (var1 > f31)
            f31 = var1;
    }
...
void camera_func_71(struct Camera *camera, struct Ball *ball)
{
    ...
    if ((debugFlags & 0xA) && camera->state == 71)
        return;
